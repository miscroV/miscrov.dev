---
title: Pwn College- Assembly HTTP Server
date: 2026-05-02 11:37:00- 0600
last_modified_at: 2026-05-02 11:37:00- 0600
categories: [Learning]
tags: [assembly]
description: Adventures in building an HTTP Server in amd64 asm. 
authors: [miscrov]
---

# Pwn.College Computing101
For the last little while I have been working on the pwn.college Computing101 module. This module is all about the very bottom of computing. Assembly. The last time I worked with Computer Science this low to the hardware I was learning about how the hardware works with the game Turing Complete!

Anyways the culminating project for this module was a functioning HTTP server able to server bot GET and POST requests in a very simplistic manner, written entirely in amd64 assembly. It took quite awhile to get the hang of building logic in asm, but after that it was largely smooth sailing to build the project. In order to keep my sanity while building the server, I used a C-like psuedo code in the comments to help me understand the logic of the asm in a simpler manner. 

To start we have the main server code, whose sole purpose is to accept connections, and then fork the process to handle it while awaiting the next connection. 
```assembly
...
The handle_request call handles whatever request is made to the main server, be it GET or POST, (returns an error 400 on other methods).
Notably, file streaming is implemented for both GET and POST rather than reading entire file into a buffer. This saves vital memory space. That is an expansion I made on the pwn.college material that was not required. 
...
handle_request:                              # void handle_request(connect_fd) {
  ...
  syscall                                    #     write(f_fd, read_buf[header_len], read_len - header_len) # write remaining of first read
                                             # 
  cmp qword ptr [rbp-32], 512                #     if (read_len >= sizeof(read_buf) { # if there is more to write after initial, stream the file. 
  jl .if3                                    # 
    .l2:                                     # 
    mov rdx, 512                             # 
    lea rsi, [rbp-1120]                      # 
...
These are just some helper functions used to process the requests. Not necessarily the most interesting. But necessary for handling and parsing the data. 
Also good practice for understanding stack frames. 
```assembly
                                             # /**
